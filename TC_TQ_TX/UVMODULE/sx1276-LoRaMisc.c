...
void SX1276LoRaSetRFFrequency( uint32_t freq )
{
    ...
    tx_WriteBuffer( REG_LR_FRFMSB, 3, &SX1276LR.RegFrfMsb );
}

uint32_t SX1276LoRaGetRFFrequency( void )
{
	tx_ReadBuffer( REG_LR_FRFMSB,3, &SX1276LR.RegFrfMsb );
...
{
	SX1276LR.RegPaConfig = tx_bSpiRead( REG_LR_PACONFIG );
	SX1276LR.RegPaDac    = tx_bSpiRead( REG_LR_PADAC  );
    ...
    tx_vSpiWrite( REG_LR_PACONFIG, SX1276LR.RegPaConfig );
...
{
	SX1276LR.RegModemConfig1  = tx_bSpiRead( REG_LR_MODEMCONFIG1 );
    SX1276LR.RegModemConfig1 = ( SX1276LR.RegModemConfig1 & RFLR_MODEMCONFIG1_BW_MASK ) | ( bw << 4 );
    tx_vSpiWrite( REG_LR_MODEMCONFIG1, SX1276LR.RegModemConfig1 );
...
void SX1276LoRaSetSpreadingFactor( uint8_t factor )
{
    ...
    SX1276LR.RegModemConfig2 = tx_bSpiRead( REG_LR_MODEMCONFIG2 );
    SX1276LR.RegModemConfig2 = ( SX1276LR.RegModemConfig2 & RFLR_MODEMCONFIG2_SF_MASK ) | ( factor << 4 );
    tx_vSpiWrite( REG_LR_MODEMCONFIG2, SX1276LR.RegModemConfig2 );
...
{
    tx_ReadBuffer( REG_LR_PREAMBLEMSB, 2, &SX1276LR.RegPreambleMsb );

    SX1276LR.RegPreambleMsb = ( value >> 8 ) & 0x00FF;
    SX1276LR.RegPreambleLsb = value & 0xFF;
    tx_WriteBuffer( REG_LR_PREAMBLEMSB, 2,  &SX1276LR.RegPreambleMsb );
}
...
void SX1276LoRaSetHopPeriod( uint8_t value )
{
    ...
    tx_vSpiWrite( REG_LR_HOPPERIOD, SX1276LR.RegHopPeriod );
    LoRaSettings.HopPeriod = value;
}

uint8_t SX1276LoRaGetHopPeriod( void )
{
	SX1276LR.RegHopPeriod = tx_bSpiRead( REG_LR_HOPPERIOD);
...
void SX1276LoRaSetPayloadLength( uint8_t value )
{
    ...
    tx_vSpiWrite( REG_LR_PAYLOADLENGTH, SX1276LR.RegPayloadLength );
    LoRaSettings.PayloadLength = value;
}

uint8_t SX1276LoRaGetPayloadLength( void )
{
	SX1276LR.RegPayloadLength = tx_bSpiRead( REG_LR_PAYLOADLENGTH );
...
void SX1276LoRaSetPa20dBm( bool enale )
{
    ...
    tx_vSpiWrite( REG_LR_PADAC, SX1276LR.RegPaDac );
}
...
{
	SX1276LR.RegPaRamp  = tx_bSpiRead( REG_LR_PARAMP);
    SX1276LR.RegPaRamp = ( SX1276LR.RegPaRamp & RFLR_PARAMP_MASK ) | ( value & ~RFLR_PARAMP_MASK );
    tx_vSpiWrite( REG_LR_PARAMP, SX1276LR.RegPaRamp );
}
...
{
	tx_ReadBuffer( REG_LR_MODEMCONFIG2, 2, &SX1276LR.RegModemConfig2 );
    ...
    tx_WriteBuffer( REG_LR_MODEMCONFIG2, 2,  &SX1276LR.RegModemConfig2 );
}
...
{
	SX1276LR.RegModemConfig3 = tx_bSpiRead( REG_LR_MODEMCONFIG3 );
    SX1276LR.RegModemConfig3 = ( SX1276LR.RegModemConfig3 & RFLR_MODEMCONFIG3_LOWDATARATEOPTIMIZE_MASK ) | ( enable << 3 );
    tx_vSpiWrite( REG_LR_MODEMCONFIG3, SX1276LR.RegModemConfig3 );
}
...
{
	SX1276LR.RegDetectOptimize  = tx_bSpiRead( 0x31);
    SX1276LR.RegDetectOptimize = ( SX1276LR.RegDetectOptimize & 0xF8 ) | value;
    tx_vSpiWrite( 0x31, SX1276LR.RegDetectOptimize );
}
