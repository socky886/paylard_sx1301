...
ncx_slab_stat(ncx_slab_pool_t *pool, ncx_slab_stat_t *stat)
{
	...
	info("pool_size : %du bytes",	stat->pool_size);
	info("used_size : %du bytes",	stat->used_size);
	info("used_pct  : %du%%\n",		stat->used_pct);

	info("total page count : %du",	stat->pages);
	info("free page count  : %du\n",	stat->free_page);
		
	info("small slab use page : %du,\tbytes : %du",	stat->p_small, stat->b_small);
	info("exact slab use page : %du,\tbytes : %du",	stat->p_exact, stat->b_exact);
	info("big   slab use page : %du,\tbytes : %du",	stat->p_big,   stat->b_big);
	info("page slab use page  : %du,\tbytes : %du\n",	stat->p_page,  stat->b_page);

	info("max free pages : %du\n",		stat->max_free_pages);
}
