...
uint16
CalcCrc (uint16 seed, const unsigned char *buf, int len)
{
  uint16 crc;
...
static uint16 ComputeCRC(uint16 crc, uint8 data, uint16 polynomial)
{
	uint8 i;
	for(i = 0; i < 8; i++)
	{
...
uint16 PacketComputerCRC(uint8* buffer, uint8 bufferlength, uint8 crcType)
{
	uint8 i;
	uint16 crc;
	uint16 polynomial;
...
uint16 CheckCrc(uint8* data, uint16 len)
{
    return PacketComputerCRC(data, len, CRC_TYPE_IBM);
}
